...
- Initialization: 
    - Draw the weights as from $w \sim N(\alpha(2h-1), \sigma_w^2), h \sim \text{Bernoulli}(0.5)$
...
- Lottery Ticket Hypothesis: 
    - [The Lottery Ticket Hypothesis](https://arxiv.org/abs/1803.03635) conjectured that, when networks are densely and randomly intialized, some sub-networks would reach test accuracy comparable to original network. In the discrete case, it would mean that some inputs are exactly mapped to the output (essentially a look up table). And a dense MLP would have implemented a soft version of the look-up table (This essentially shows again that all models are K-Nearest ). The Boolean MLP might be an interesting model to probe the Lottery Ticket Hypothesis further.
- Pruning and 1-bit LLMs
    - In the context Large Language Models (LLMs), we see large models with parameters in the order of billions are successfully compressed/quantized without much degradation in the performance. See for example, [The Era of 1-bit LLMs. All LLMs are 1.58 bits](https://arxiv.org/abs/2402.17764). If all the weights are sampled from \{-1,0,+1}\ they require $\log_2(3)=1.58$ bits. 
...
- Hardware Accelerators
    ...
    - GPUs are the backbone of compute infra to train Deep Neural Nets. At the heart of it, it is the matrix multiplication that needs to be done efficiently.  What if we networks have no explicit MatMul operations - see [Scalable MatMul-free Language Modeling](https://arxiv.org/abs/2406.02528) for an approach or the more recent [BOLD: Boolean Logic Deep Learning](https://arxiv.org/abs/2405.16339) where networks are trained based on a new mathematical principle of Boolean Variation with considerations for chip architecture, memory hierarchy, dataflow and arithmetic precision. 
    - Can we directly burn the model onto silicon? That is, can we take a Boolean MLP specified in PyTorch and create HDL files using the latest and greatest in VLSI technology? See [hls4ml](https://github.com/fastmachinelearning/hls4ml).
