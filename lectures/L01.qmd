...
\end{array}
$$
where ${\bf  b}$ represents the ${n_0} \times 1$ bias (or intercept) term, ${\bf w}$ is the weight matrix (regression coefficients) and ${\bf X}$ is the set of all $N \times p$ features excluding the column of ones (which was included to model the intercept/ bias term).
...
The extension of the logistic regression to multiclass is straightforward but we need to different way of writing the models. The Binary Classification problem can written in the following equivalent fashion:
$$
\begin{array}{left}
y^{[i]} \sim Bin(\pi^{[i]}, 1)\\
...
It is like $f(x^{[i]})$ is now modeling the difference in the class-specific responses. To make this explicit, suppose, $f_k(x^{[i]})$ is the class-specific latent response (whose thresholded version yield the binary response as note before). Then, by specification, $f(x^{[i]}) =  f_1(x^{[i]}) - f_0(x^{[i]})$. It follows then that,
$$
\begin{array}{left}
...
Now we are in a position to extend the Binary Classification problem to the Multi-class setting, where the scales are nominal (i.e., the class labels have no ordering, and labels could have been shuffled with no consequence). 
...
When the constrains are removed, and  $f_k(x)= {\bf x}{\bf w_k} + {\bf  b_k}$ is a linear model, we get a Vector Generalized Linear Model ([VGLAM](https://en.wikipedia.org/wiki/Vector_generalized_linear_model)). VGLAM is shown as a (shallow) network below.
...
Let us consider boolean variables and just two inputs $x_1, x_2 \in \{0,1\}$. We will model the popular logic gates using a simple Neuron (strictly speaking, a McCulloh-Pitts Neuron): $y = H\left(x_1 w_1 + x_2 w_2 + b\right)$, where  the Heavyside step function $H(x) = 1 \text{ if } x\ge 0, 0. \text{ o.w}$ 
...
Observe that, the gate is hot only when all $x \in A$ are 1s and $x \in A^c$ are 0s. 

Case | $x \in A$ | $x \in \bar{A}$ | $y_{AANOR}$ |
|-----|-----|-----|-----|-----|
Case-1| All 1s| All 0s | 1 |
Case-2| At least one 0| Any | 0 |

Verify that cases are mutually exclusive and exhaustive. Based on our idea about AND and NOR gates, let us hypothesize the following Neuron for the AANOR gate as follows:
$$
\begin{array}{left}
y &=& H( \sum_{i \in A} (x_i-1) -0.5 + \sum_{i \in \bar{A}} -x_i + 0.5 + b) \\
y &=& H( \sum_{i \in A} (x_i-1) + \sum_{i \in \bar{A}} -x_i + b)
\end{array}
$$
where $b$ is to found out.

Case-1:
In this case, substituting the specific x's, we get the inequality $b > 0$

Case-2:
At least one x in A is 0, which means, we need 
$\max \left( \sum_{i \in A} (x_i-1) + \sum_{i \in \bar{A}} -x_i + b \right) < 0$.  It is $1+b$ and is achieved when exactly one x in A is zero, and all x's in $\bar{A}$ are zero. Therefore, we get, $b-1 < 0$.
From both the inequalities, we can choose $b=0.5$. The AANOR gate can now be realized as:
$$
\begin{array}{left}
y &=& H( \sum_{i \in A} (x_i-1) + \sum_{i \in \bar{A}} -x_i + 0.5)
\end{array}

Now that we can realize any AANOR gate, and OR gate is something we have already seen, any M-ary Boolean Truth Table can be realized
$$
\begin{array}{left}
y &=& OR(\{ AANOR(x^{[i]} \}), i \in \{ i' \text{ s. t } y^[i']=1\}
\end{array}
$$
Above can be seen a composition of Boleean gates: 
$$
\text{ inputs > ANOR > OR }
$$
an MLP with 1-hidden layer in hindsight, as illustrated in the figure below.
![SOP = MLP with 1-hidden layer](./../figs/FFNs-SOPs.drawio.png){fig-align="center"}

Effectively, we exploited the fact that any M-ary Truth Table can be expressed in SOP form, and we have constructed a 1-hidden layer MLP which can exactly models the SoP.

Of course, we are operating on Boolean inputs. The continuous analogue of what we derived for MLPs with 1-hidden layer are due to the early works of Cybenko's [Universal Approximation Theorem](https://hal.science/hal-03753170/), [pdf](https://web.njit.edu/~usman/courses/cs675_fall18/10.1.1.441.7873.pdf). A nice illustration of the Universal Approximation abilities of MLPs can be found here - [Representational Power of NNs](http://neuralnetworksanddeeplearning.com/chap4.html)

### Summary

- Logistic regression is member of Generalized Linear Models (GLMs)
- Generalized Linear Models is member of larger class Vector Generalized Linear Models (VGLMs)
- Logistic regression = Perceptron with sigmoid activation
- MLP with an input and output layer (with multiple outputs) = VGLM
- MLPs are Deep VGLMS
- Any M-ary Boolean circuits can be represented in Sum-of-Products form
- AND, OR, NAND, NOR gates can be relaized by percetrons.
- Sum-of-Products can be relaized by an MLP with 1-hidden layer.
